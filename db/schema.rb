...
ActiveRecord::Schema.define(version: 20181219145520) do
  ...
  create_table "ci_builds", force: :cascade do |t|
    ...
    t.index ["commit_id", "artifacts_expire_at", "id"], name: "index_ci_builds_on_commit_id_and_artifacts_expireatandidpartial", where: "(((type)::text = 'Ci::Build'::text) AND ((retried = false) OR (retried IS NULL)) AND ((name)::text = ANY ((ARRAY['sast'::character varying, 'dependency_scanning'::character varying, 'sast:container'::character varying, 'container_scanning'::character varying, 'dast'::character varying])::text[])))", using: :btree
  ...
  create_table "ci_pipelines", force: :cascade do |t|
    ...
    t.index ["project_id", "ref", "id"], name: "index_ci_pipelines_on_project_id_and_ref_and_id_desc", order: { id: :desc }, using: :btree
